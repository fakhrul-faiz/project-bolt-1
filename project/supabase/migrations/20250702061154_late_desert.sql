/*
  # Initial GambarKaca Database Schema

  1. New Tables
    - `profiles` - User profiles with role-based data
    - `campaigns` - Marketing campaigns created by founders
    - `campaign_applications` - Talent applications to campaigns
    - `orders` - Orders created when talents are approved
    - `transactions` - Financial transactions for wallet and payments
    - `earnings` - Talent earnings tracking
    - `messages` - Chat messages between founders and talents
    - `reviews` - Review submissions from talents
  ...
  3. Functions
    - Handle campaign pricing calculations
    - Manage wallet balance updates
